...
class CnnViT(nn.Module):
    def __init__(self, P, L, size, patch, dim, col):
        ...
        self.vtrans = VisionTransformer(img_size=col,
                              patch_size=patch,
                              in_c=(dim * P) // patch ** 2,
                              num_classes=P,
                              embed_dim=P*dim,
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        这里和vit in HSU不一样。
        HSU只是简单的矩阵变换；
        这里利用了2d卷积进行patch分解，卷积核大小和patch大小一样。
        '''
...
if __name__ == '__main__':
    ...
    net = VisionTransformer(img_size=input_shape[1], in_c=input_shape[0]).cuda()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dummy_input = torch.randn(1, input_shape[0], 110, 110).to(device)
