...
class CnnSwinTransformer(nn.Module):
    def __init__(self, P, L, size, patch, dim, col, checkpoint):
        super(CnnSwinTransformer, self).__init__()
        ...
        self.encoder = nn.Sequential(
            ...
            # nn.Conv2d(64, 3, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)),
            # nn.BatchNorm2d(3, momentum=0.5),


        )

        self.swintrans = SwinTransformer(patch_size=4,
                                         in_chans=L,
                                         num_classes=P,
                                         embed_dim=P * dim,
                                         depths=(2, 2, 6, 2),
        ...
        # (2**3)是因为经过了三次swinblock，通道扩张了三次2倍
        self.upscale = nn.Sequential(
    ...
    def forward(self, x):
        ...
        cls_emb = self.swintrans(x)
        # 其实这里可以直接输出为1*(P*size*size)?
        # 不行，因为想要输出为(P*size*size)就需要输入的patch partition的dim为(P*size*size)/2/2/2/2，samson：dim=1128，太大了
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # self.head = nn.Linear(self.num_features, 3*95*95) if num_classes > 0 else nn.Identity()
